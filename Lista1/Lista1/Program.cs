...
//Objetivo: usando uma aplicação do tipo console do dotnet, criar uma aplicação que
//converterá o real ( R$ ) para as moedas dólar ( $ ), euro ( € ), iene ( ¥ ) e libra esterlina ( £ ).
//Legenda
...
//Novos conceitos ou funcionalidades desse exercício
//Enum (Enumeration types) pode ser utilizado para controlar as moedas, estude como usar
//enumeradores no C#. Usaremos nesse exercício a classe System.Globalization.CultureInfo.
//Você pode customizar a exibição do valor da moeda usando as conversões de string sem
//utilizá-la, mas o ideal é conhecer o CultureInfo, isso facilitará seu trabalho, então estude
//como formatar as moedas utilizando-a. Estude também sobre os tipos mais indicados para
//moedas e qual o motivo. Durante o exercício dê preferência para o decimal, pois geralmente
//ele é o mais indicado na maioria das situações. Lembre-se de utilizar os conceitos já
//utilizados nos exercícios anteriores.
//Estude:
...
//● Após leitura do valor a ser convertido, exibir um menu para selecionar para qual
//moeda o usuário deseja converter: dólar($), euro(€), iene(¥) ou libra esterlina (£).
...
//● Fazer o controle de versionamento da sua aplicação usando git;
//● Subir a aplicação para um repositório do github.
//● Estude o uso de Dictionary e utilize-o para armazenar o valor de cada moeda que
//será usado na conversão.
//Desafios

//1. Crie na sua aplicação a opção de converter as moedas dólar ( $ ), euro ( € ), iene ( ¥
//) e libra esterlina ( £ ) em real ( R$ ).
//2. Permita que o usuário informe qualquer uma das moedas (real , dólar, euro, iene ou
//libra esterlina) e depois converta pra qualquer uma das opções restantes
